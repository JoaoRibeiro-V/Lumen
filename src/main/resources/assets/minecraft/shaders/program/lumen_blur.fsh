...
float gaussianPdf(in float x, in float n) {
    return 0.39894 * exp( -0.5 * x * x/( n * n))/n;
}

void main(){
    vec2 invSize = 1.0 / OutSize;
    float fN = float(Radius);
    float weightSum = gaussianPdf(0.0, fN);
    vec3 diffuseSum = texture(DiffuseSampler, texCoord).rgb * weightSum;
    for( int i = 1; i < Radius; i ++) {
        float x = float(i);
        float w = gaussianPdf(x, fN);
